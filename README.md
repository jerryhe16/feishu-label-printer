...
支持 **物料标签**（Code 128料号条码、料号、名称、规格）和 **样机标签**（Logo、点阵编号、当前记录二维码）。当前版式针对 **ZD888 300 dpi、30×20 mm**，支持毫米级上下偏移和超出边界检查。
...
当前限制：单个打印队列只能由一个电脑/程序消费；没有分布式抢占。只实现300 dpi、30×20 mm版式。状态“已发送”表示Windows接收了打印任务，不保证纸张已经输出。长料号或过长文字会明确拒绝排版，不自动截断。
